import { db } from '..';
import { DatabaseEventTypes, TaskDBEventTypes, TransactionTypes } from '../../../schema/DatabaseTypes';
...
class TaskService extends EventListener<TaskDBEventTypes>{
  ...
  async createTable(upgradeDB:any){
    let taskObjectStore:any = upgradeDB.createObjectStore(this.tableName, { keyPath: 'id', autoIncrement: true });
  ...
  async addTask(task: Task){
    try{
      ...
      let transaction:any = this.getTransactionFromDb(idb, TransactionTypes.READ_WRITE);
  ...
  async getAllTasks(){
    try{
      const idb = db.getIDBInstance();
      if(idb){
        let transaction:any = this.getTransactionFromDb(idb, TransactionTypes.READ_ONLY);
        let store:any = this.getObjectStore(transaction);
        return store.getAll();
      }
    }
